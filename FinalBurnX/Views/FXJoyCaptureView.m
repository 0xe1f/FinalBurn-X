...
@implementation FXJoyCaptureView
...
+ (void) initialize
{
	...
	NSDictionary *dict = @{ @"Up": @(FXGamepadUp),
							@"Down": @(FXGamepadDown),
							@"Left": @(FXGamepadLeft),
							@"Right": @(FXGamepadRight) };
...
- (void) mouseDown:(NSEvent *) theEvent
{
    ...
    if ([self canUnmap]) {
        ...
        if (NSPointInRect(mousePosition, [self unmapRect])) {
            [self captureCode:FXGamepadNone];
        }
    }
}
...
{
    NSString *name = [FXJoyCaptureView descriptionForCode:code];
    if (!name) {
        name = @"";
    }
